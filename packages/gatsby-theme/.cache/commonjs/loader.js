...
const prefetchResource = resourceName => {
  if (resourceName.slice(0, 12) === `component---`) {
    return Promise.all(
      createComponentUrls(resourceName).map(url => (0, _prefetch.default)(url))
    )
  } else {
    const url = createJsonURL(jsonDataPaths[resourceName])
...
const shouldFallbackTo404Resources = path =>
  (hasPageResourceMap || inInitialRender) && path !== `/404.html` // Note we're not actively using the path data atm. There
